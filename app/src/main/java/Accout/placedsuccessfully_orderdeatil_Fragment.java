...
//    private String ngaydat[] = {"30/10/2024", "22/10/2024", "1/10/2024", "8/8/2024","1/8/2024"};
    ...
    private void loadFirebaseid() {
        ...
        // Load dish names
        mDatabase.child("DonHang").child(iduser+"").child("2").addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
    ...
    private void loadNgay(List<String> id) {
        ...
        for(int i = 0 ; i< id.size() ;i++) {
            ...
            mDatabase.child("DonHang").child(iduser + "").child("2").child(id.get(i)).child("Date").addListenerForSingleValueEvent(new ValueEventListener() {
                @Override
                public void onDataChange(@NonNull DataSnapshot snapshot) {
                    date.add(snapshot.getValue(String.class));
                    if (id.size() == date.size()) {
                        for (int i = (id.size() - 1); i >= 0; i--) {
                            list.add(new Class_sumdeltaihistory(id.get(i), 2, date.get(i)));
                        }
